...
it('creates feed items with correct sequence numbers', function () {
    ...
    $job->handle(app(FeedItemOrderingService::class));
...
it('is idempotent and assigns the next sequence to new attachments', function () {
    ...
    expect(FeedItem::where('feed_id', $feed->id)->where('library_item_id', $libraryItem->id)->count())->toBe(1)
        ->and(FeedItem::where('feed_id', $feed->id)->where('library_item_id', $libraryItem->id)->value('sequence'))->toBe(0);

    $nextLibraryItem = LibraryItem::factory()->create(['user_id' => $user->id]);
    (new AddLibraryItemToFeedsJob($nextLibraryItem, [$feed->id]))->handle(app(FeedItemOrderingService::class));
